...
from .pipe_client import get_service_info_via_pipe, search_files_via_pipe, test_pipe_connection
...
async def search_files(
    pattern: str, directory: str = ".", max_results: int = 100
) -> List[Dict[str, Any]]:
...
async def _fallback_search(
    ...
    try:
        ...
        logger.info(
            f"Fallback search found {len(results)} files matching '{pattern}' in '{directory}'"
        )
        return results
