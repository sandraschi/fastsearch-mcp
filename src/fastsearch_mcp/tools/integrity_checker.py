...
@tool(
    ...
    parameters=[
        ...
        ToolParameter(
            name="recursive", type=bool, description="Recursively check directories", default=True
        ),
        ToolParameter(
            name="patterns",
            type=list,
            ...
            if os.path.isfile(path):
                ...
                try:
                    hash_value = await asyncio.get_event_loop().run_in_executor(
                        None, self._hash_file, path, algorithm
                    )
                ...
                for pattern in patterns:
                    for file_path in Path(path).rglob(pattern):
                        ...
                        try:
                            hash_value = await asyncio.get_event_loop().run_in_executor(
                                None, self._hash_file, file_path_str, algorithm
                            )
