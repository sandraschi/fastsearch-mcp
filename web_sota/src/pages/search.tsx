...
const CATEGORY_EXTENSIONS: Record<string, string[]> = {
    Code: ["py", "ts", "tsx", "js", "jsx", "cpp", "c", "h", "cs", "java", "rs", "go", "json", "html", "css", "xml", "yml", "yaml", "sh", "ps1"],
...
function getFileIcon(filename: string) {
    ...
    if (CATEGORY_EXTENSIONS.Code.includes(ext)) return <FileCode className="h-4 w-4 text-blue-400 shrink-0" />;
    if (CATEGORY_EXTENSIONS.Images.includes(ext)) return <FileImage className="h-4 w-4 text-emerald-400 shrink-0" />;
    if (CATEGORY_EXTENSIONS.Media.includes(ext)) return <FileVideo className="h-4 w-4 text-purple-400 shrink-0" />;
    if (CATEGORY_EXTENSIONS.Archives.includes(ext)) return <Archive className="h-4 w-4 text-amber-400 shrink-0" />;
    if (CATEGORY_EXTENSIONS.Docs.includes(ext)) return <FileText className="h-4 w-4 text-slate-300 shrink-0" />;
...
export function Search() {
    ...
    const [history, setHistory] = useState<string[]>(() => {
        try {
            ...
            return raw ? JSON.parse(raw) : ["*.py", "*.pdf", "*.log", "*.docx", "*config*"];
        } catch {
    ...
    const handleStartService = async () => {
        ...
        try {
            ...
            setSearchError(e instanceof Error ? e.message : "Failed to start service");
        } finally {
            setServiceLoading(false);
        }
    };
    ...
    const runSearch = async (overridePattern?: string, overrideDir?: string) => {
        ...
        // Update history
        setHistory((prev) => {
            const updated = [activePattern, ...prev.filter((p) => p !== activePattern)].slice(0, 10);
            try {
        ...
        try {
            ...
            if (res.service_down) {
                setServiceRunning(false);
                setSearchError(res.error || "FastSearch Windows Service is not running.");
                setResults([]);
            ...
            setSearchError(e instanceof Error ? e.message : "Unexpected search error");
            setResults([]);
        } finally {
            setSearching(false);
        }
    ...
    const filteredResults = useMemo(() => {
        return results.filter((item) => {
            ...
            if (filterQuery && !name.includes(filterQuery.toLowerCase()) && !fullPath.includes(filterQuery.toLowerCase())) {
                return false;
            }
                    ...
                    {results.length > 0 && (
                        <div className="pt-3 flex flex-col sm:flex-row gap-3 justify-between items-center">
                            ...
                            <div className="flex items-center gap-1 overflow-x-auto w-full sm:w-auto py-1">
                                {["All", "Code", "Docs", "Images", "Media", "Archives", "Apps"].map((cat) => (
                                    <button
                                        key={cat}
                                        onClick={() => {
                ...
                <CardContent className="p-0">
                    {searching ? (
                        <div className="flex flex-col items-center justify-center py-16 text-slate-400">
                            ...
                            <p className="text-sm font-medium">Scanning Master File Table...</p>
                        </div>
                    ) : results.length === 0 ? (
                        <div className="flex flex-col items-center justify-center py-16 text-slate-500">
                            <SearchIcon className="h-10 w-10 text-slate-700 mb-3" />
                            <p className="text-sm font-medium text-slate-400">No search results</p>
                        ...
                        <div className="flex flex-col items-center justify-center py-12 text-slate-500">
                            <p className="text-sm font-medium text-slate-400">No files match the active filters</p>
                            <Button
                                variant="link"
                                ...
                                <tbody className="divide-y divide-slate-800/60 font-mono text-slate-300">
                                    {paginatedResults.map((item, idx) => {
                                        ...
                                        return (
                                            ...
                                            >
                                                <td className="py-2.5 px-4 text-slate-500">{globalIdx}</td>
                    ...
                    <div className="w-full max-w-2xl bg-slate-950 border-l border-slate-800 h-full flex flex-col shadow-2xl animate-in slide-in-from-right duration-200">
                        ...
                        <div className="p-4 border-b border-slate-800 flex items-center justify-between">
                            <div className="flex items-center gap-2 truncate">
                                ...
                                <span className="font-semibold text-slate-200 truncate">{previewFile.path}</span>
                            </div>
                            <Button
                                variant="ghost"
                                size="sm"
                        ...
                        <div className="bg-slate-900/60 px-4 py-2 text-xs text-slate-400 flex items-center gap-4 border-b border-slate-800">
                            ...
                            {previewFile.truncated && <span className="text-amber-400">(Truncated preview)</span>}
                        </div>
                        ...
                        <div className="flex-1 overflow-auto p-4 font-mono text-xs text-slate-300 bg-slate-900/30">
                            {previewLoading ? (
                                <div className="flex items-center justify-center h-48 text-slate-400 gap-2">
                                    <Loader2 className="h-5 w-5 animate-spin text-blue-500" /> Loading preview...
                                </div>
