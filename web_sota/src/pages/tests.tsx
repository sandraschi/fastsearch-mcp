...
import { CheckCircle2, XCircle, Loader2, Play, FlaskConical } from "lucide-react";
...
export function Tests() {
    ...
    const runTests = async () => {
        ...
        setResult(null);
        try {
            const res = await mcpClient.runTests({ pattern, directory, max_results: maxResults });
            setResult(res);
        } catch (e: unknown) {
        ...
        <div className="space-y-6">
            <div>
                ...
                <p className="text-slate-400">
                    Run integration tests against the FastSearch service: pipe connection, service info, and real search via pipe.
                </p>
            </div>
            ...
            <Card className="border-slate-800 bg-slate-950/50">
                <CardHeader>
                    ...
                    <CardDescription className="text-slate-400">
                        Used for the &quot;search_via_pipe&quot; test (pattern, directory, max_results).
                    </CardDescription>
                </CardHeader>
                <CardContent className="space-y-3">
                    <div className="grid gap-3 sm:grid-cols-3">
                        <div>
                            <label className="text-xs font-medium text-slate-400">Pattern</label>
                            <input
                                type="text"
                                value={pattern}
                        ...
                        <div>
                            <label className="text-xs font-medium text-slate-400">Directory</label>
                            <input
                                type="text"
                                value={directory}
                        ...
                        <div>
                            <label className="text-xs font-medium text-slate-400">Max results</label>
                            <input
                                type="number"
                                min={1}
                                max={100}
