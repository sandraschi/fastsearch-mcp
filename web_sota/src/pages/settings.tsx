...
export function Settings() {
  ...
  const loadModels = async () => {
    ...
    try {
      ...
      const res = await mcpClient.getLlmModels(
        config.provider,
        config.baseUrl || undefined,
      );
      setModels(res.models || []);
      if (res.error) setModelsError(res.error);
      if (!config.model && res.models?.length)
        setConfig((c) => ({ ...c, model: res.models[0] }));
    } catch (e) {
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-slate-400">
          Manage indexing behavior and local LLM integration
        </p>
      </div>
      ...
      <div className="grid gap-6">
        <Card className="border-slate-800 bg-slate-950/50">
          <CardHeader>
            ...
            <CardDescription className="text-slate-400">
              Provider, model discovery, and base URL for Chat and Advanced
              Result Analysis
            </CardDescription>
          </CardHeader>
              ...
              <div className="flex gap-2">
                ...
                >
                  <SelectTrigger className="bg-slate-900 border-slate-800 text-slate-100 flex-1">
                    <SelectValue
                      placeholder={
                        loadingModels ? "Discovering..." : "Select model"
                      }
                    />
                  </SelectTrigger>
                  <SelectContent className="bg-slate-900 border-slate-800 text-slate-100">
            ...
            <div className="flex justify-end">
              ...
              >
                {saved ? (
                  ...
                  <>
                    <Save className="mr-2 h-4 w-4" /> Save LLM config
                  </>
                )}
              </Button>
            </div>
        ...
        <Card className="border-slate-800 bg-slate-950/50">
          <CardHeader>
            ...
            <CardDescription className="text-slate-400">
              File system monitoring and performance
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="grid gap-2">
              <Label className="text-slate-300 font-semibold text-xs uppercase tracking-wider">
                FastSearch Utility Path
              </Label>
              <Input
                className="bg-slate-900 border-slate-800 text-slate-100 placeholder:text-slate-500"
