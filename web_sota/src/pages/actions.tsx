...
export function Actions() {
  ...
  const handleScanRepos = async () => {
    ...
    setResults(null);
    setStatus(
      "Scanning d:/Dev/repos for package.json (repository identification)...",
    );
    try {
      const response = await mcpClient.callTool("fastsearch_search_advanced", {
      ...
      if (response && response.results) {
        setResults(response.results);
        setStatus(
          `Identified ${response.results.length} active Node.js repositories.`,
        );
      } else {
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-slate-400">
          High-impact indexing and discovery tasks
        </p>
      </div>
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        <Card className="border-slate-800 bg-slate-950/50">
          <CardHeader>
            ...
            <CardDescription className="text-xs text-slate-400">
              Scan d:/Dev/repos/ for folders containing "webapp", "frontend", or
              "ui".
            </CardDescription>
          </CardHeader>
          <CardContent>
        ...
        <Card className="border-slate-800 bg-slate-950/50">
          <CardHeader>
            ...
            <CardDescription className="text-xs text-slate-400">
              Identify active development repositories by locating project
              manifest files.
            </CardDescription>
          </CardHeader>
          <CardContent>
        ...
        <Card className="border-slate-800 bg-slate-950/50">
          ...
          <CardContent>
            <Button
              variant="outline"
              className="w-full border-slate-800 text-purple-400 hover:bg-purple-950/30"
            >
              Run Cleanup
            </Button>
          </CardContent>
            ...
            {results && (
              <div className="max-h-[300px] overflow-y-auto space-y-2 pr-2 custom-scrollbar">
                {results.map((res, idx) => (
                  <div
                    key={idx}
                    className="text-[10px] font-mono p-2 rounded bg-slate-950 border border-slate-800 text-slate-300 flex justify-between"
                  >
        ...
        <CardContent className="space-y-4">
          <div className="flex items-center justify-between text-sm">
            ...
            <span className="text-slate-100 font-mono">
              D:\Dev\repos; C:\Users\sandr\.gemini\antigravity
            </span>
          </div>
          <div className="flex items-center justify-between text-sm">
