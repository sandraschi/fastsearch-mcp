...
export function Service() {
    ...
    const fetchStatus = useCallback(async () => {
        ...
        setActionError(null);
        try {
            const res = await mcpClient.callTool("service_status", { level: "intermediate" }) as ServiceStatus;
            setStatus(res);
        } catch (e) {
            setStatus({
                success: false,
        ...
        <div className="space-y-6">
            <div>
                ...
                <p className="text-slate-400">
                    Windows service for direct NTFS Master File Table access. Install via MSI; start/stop/restart require admin.
                </p>
            </div>
                        ...
                        <>
                            <div className="grid gap-4 sm:grid-cols-2">
                                <div className="flex items-center gap-3 rounded-lg border border-slate-800 bg-slate-900/50 p-4">
                                    ...
                                    <div>
                                        <p className="text-sm font-medium text-slate-200">Process</p>
                                        <p className="text-xs text-slate-400">
                                            {running ? "Running" : "Stopped"}
                                            {status?.service_state ? ` (${status.service_state})` : ""}
                                        </p>
                                    </div>
                                </div>
                                <div className="flex items-center gap-3 rounded-lg border border-slate-800 bg-slate-900/50 p-4">
                                    ...
                                    <div>
                                        <p className="text-sm font-medium text-slate-200">Named pipe</p>
                                        <p className="text-xs text-slate-400">
                            ...
                            <p className="text-xs text-slate-500">
                                Start/Stop/Restart require administrator privileges. Install the service via fastsearch-mcp-setup.msi (see README).
                            </p>
                        </>
                    )}
                </CardContent>
            ...
            <Card className="border-slate-800 bg-slate-950/50">
                <CardHeader>
                    ...
                    <CardDescription className="text-slate-400">
                        Run a tiny search (C:\*.txt, max 5) to confirm the service returns results at speed
                    </CardDescription>
                </CardHeader>
                <CardContent>
                    <Button
                        ...
                        <div className="mt-4 rounded-lg border border-slate-800 bg-slate-900/50 p-4">
                            {probe.error ? (
                                ...
                                <p className="text-sm text-slate-200">
                                    Latency: <span className="font-mono text-emerald-400">{probe.latencyMs} ms</span>
                                    {" · "}
