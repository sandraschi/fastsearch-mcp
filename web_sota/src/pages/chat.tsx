...
const PERSONALITIES: Record<string, string> = {
  "Search Expert":
    "You are a search expert specializing in high-performance file indexing and search. Provide guidance on search patterns, MFT queries, and file system navigation.",
  "Data Analyst":
    "You are a data analyst focused on file system analytics. Help with file patterns, storage analysis, and data organization strategies.",
...
const EXAMPLE_PROMPTS = [
  {
    group: "Search",
    prompts: [
      "Find all .pdf files larger than 50MB",
  ...
  },
  {
    ...
    prompts: [
      ...
      "Reindex the Documents folder",
      "Check search database health",
    ],
  },
  {
    group: "Files",
...
export function Chat() {
  const [personality, setPersonality] = useState(
    () => localStorage.getItem(PERSONALITY_KEY) || "Search Expert",
  );
  ...
  const exportChat = () => {
    ...
    a.href = url;
    a.download = "fastsearch-chat.txt";
    a.click();
      ...
      >
        <div>
          ...
          <p className="text-slate-400">
            Local LLM chat with prompt refinement for file search guidance
          </p>
        </div>
        <div className="flex items-center gap-2">
          <span className="text-xs text-slate-500 bg-slate-800 px-2 py-1 rounded">
            skill:fastsearch-expert
          </span>
          <span className="text-sm text-slate-400">
            Model: {getLlmConfig().model || "Not set"}
          </span>
          <select
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 flex-1 min-h-0">
        <div className="lg:col-span-1 border border-slate-800 bg-slate-950/50 rounded-lg flex flex-col overflow-hidden">
          ...
          <div className="p-4 flex-1 overflow-hidden flex flex-col">
            <textarea
              className="flex-1 min-h-[120px] bg-slate-900 border border-slate-800 text-slate-200 text-sm rounded p-2 resize-none"
              placeholder="Define how the assistant should behave..."
              value={PERSONALITIES[personality]}
              onChange={() => {
                /* Personality-managed */
              }}
              onBlur={() =>
                setLlmConfig({ systemPrompt: PERSONALITIES[personality] })
              }
            />
            <p className="text-xs text-slate-500 mt-2">
              Managed by personality selector above.
            </p>
          </div>
        </div>
        ...
        <div className="lg:col-span-2 border border-slate-800 bg-slate-950/50 rounded-lg flex flex-col overflow-hidden">
          ...
          >
            {messages.length === 0 && (
              <div className="text-center text-slate-500 py-8">
                ...
                <p>
                  Ask for search suggestions, patterns, or follow-up actions.
                </p>
                <p className="text-xs mt-2">
                  Example: &quot;Find all .pdf files larger than 50MB in
                  D:\Documents&quot;
                </p>
              </div>
            )}
            {messages.map((msg, i) => (
              ...
              >
                <div
                  className={`w-8 h-8 rounded-full shrink-0 flex items-center justify-center ${msg.role === "assistant" ? "bg-blue-600/20 border border-blue-500/30" : "bg-slate-800 border border-slate-700"}`}
                >
                  {msg.role === "assistant" ? (
                ...
                <div
                  className={`flex-1 space-y-1 ${msg.role === "user" ? "text-right" : ""}`}
                >
                  <p className="text-xs font-medium text-slate-400">
                    {msg.role === "user" ? "You" : "Search AI"}
                  </p>
          ...
          <div className="p-4 border-t border-slate-800 bg-slate-900/20">
            ...
            <div className="flex gap-2">
              <textarea
                className="min-h-[44px] max-h-32 flex-1 bg-slate-900 border border-slate-800 text-slate-100 placeholder:text-slate-500 rounded p-2 text-sm resize-none"
              ...
              <button
                ...
                onClick={send}
                disabled={loading}
                className="bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white px-4 py-2 rounded-md shrink-0"
              >
                {loading ? (
