...
from fastsearch_mcp.mcp_server import McpServer  # noqa: E402

# Configure logging for tests
logging.basicConfig(
    level=logging.DEBUG,
    format="%(asctime)s - %(name)s - %(levelname)s - %(message)s",
    handlers=[logging.StreamHandler(), logging.FileHandler("test_mcp_server.log")],
)
...
async def test_handle_search(server, mock_client):
    ...
    mock_client.search.assert_called_once_with(
        pattern="test", search_type="fuzzy", max_results=2, **{}
    )
...
async def test_register_tool(server):
    ...
    result = await server._execute_handler(
        {"jsonrpc": "2.0", "method": "test.tool", "params": {"arg1": "test", "arg2": 123}, "id": 1}
    )
